...
public protocol CropViewDelegate : class {
     ...
     */
    func cropViewDidStartCrop(_ cropView: CropView)
    
    /*
     Calls always, when user move touch around view
     */
    func cropViewDidMove(_ cropView: CropView)
    
    /*
     Calls ones, when user stop interaction with view
     */
    func cropViewDidStopCrop(_ cropView: CropView)
    
    /*
     Calls ones, when change a Crop frame
     */
    func cropViewInsideValidFrame(for point: CGPoint, from cropView: CropView) -> Bool
}
...
public class CropView: UIView {
    ...
    internal lazy var horizontalCropLines: [CropLine] = { [unowned self] by in
        var lines = self.setupHorisontalLines(count: self.cropLinesCount,
                                              className: CropLine.self)
        return lines as! [CropLine]
        }(())
    
    internal lazy var verticalCropLines: [CropLine] = { [unowned self] by in
        var lines = self.setupVerticalLines(count: self.cropLinesCount,
                                            className: CropLine.self)
        return lines as! [CropLine]
        }(())
    
    internal lazy var horizontalGridLines: [CropGridLine] = { [unowned self] by in
        var lines = self.setupHorisontalLines(count: self.gridLinesCount,
                                              className: CropGridLine.self)
        return lines as! [CropGridLine]
        }(())
    internal lazy var verticalGridLines: [CropGridLine] = { [unowned self] by in
    ...
    fileprivate func setup() {
        ...
        let upperLeft = CropCornerView(cornerType: .upperLeft,
        ...
        let upperRight = CropCornerView(cornerType: .upperRight,
                                           lineWidth: cornerBorderWidth,
                                           lineLenght:cornerBorderLength)
        ...
        let lowerRight = CropCornerView(cornerType: .lowerRight,
                                           lineWidth: cornerBorderWidth,
                                           lineLenght:cornerBorderLength)
        ...
        let lowerLeft = CropCornerView(cornerType: .lowerLeft,
                                          lineWidth: cornerBorderWidth,
                                          lineLenght:cornerBorderLength)

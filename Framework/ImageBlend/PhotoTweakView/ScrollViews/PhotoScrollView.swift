...
public protocol PhotoScrollViewDelegate : class{
    /*
     Calls ones, when user start interaction with view
     */
    func scrollViewDidStartUpdateScrollContentOffset(_ scrollView: PhotoScrollView)
    
    /*
     Calls ones, when user stop interaction with view
     */
    func scrollViewDidStopScrollUpdateContentOffset(_ scrollView: PhotoScrollView)
}
...
public class PhotoScrollView: UIScrollView {
     ...
     */
    weak var updateDelegate: PhotoScrollViewDelegate?
    
    //MARK: - Protected VARs
    
    init(frame: CGRect, contentSize: CGSize) {
        super.init(frame: frame)
        setup(contentSize: contentSize)
    }
    
    required public init?(coder aDecoder: NSCoder) {
        super.init(coder: aDecoder)
        let size = CGSize(width: self.bounds.size.width, height: self.bounds.size.height)
        setup(contentSize: size)
    }
    
    fileprivate func setup(contentSize: CGSize) {
        self.bounces = true
    ...
    override public var contentOffset: CGPoint {
        set {
            ...
            PhotoScrollView.cancelPreviousPerformRequests(withTarget: self,
                                                             selector: selector,
                                                             object: nil)
